...
{block name='frontend_listing_box_article_image_picture_element'}
	...
	{if $theme.cbax_previewimagechange_active && $theme.cbax_previewimagechange_show != 'emotion'}
		{if $cbaxMarsImages[1] && $theme.cbax_previewimagechange_effect == 'change'}
			<img class="cbax--preview-image--1" srcset="{$cbaxMarsImages[0].thumbnails[1].sourceSet}"
				 alt="{$desc}"
				 title="{$desc|truncate:25:""}"/>
			<img class="cbax--preview-image--2" srcset="{$cbaxMarsImages[1].thumbnails[1].sourceSet}"
				 alt="{$desc}"
				 title="{$desc|truncate:25:""}"/>
